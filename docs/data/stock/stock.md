...
描述: 获取东方财富网-数据中心-龙虎榜单-营业部详情，获取特定营业部的交易明细数据
...
| 名称        | 类型  | 必选 | 描述                    |
|-----------|-----|----|-----------------------|
| dept_code | str | Y  | dept_code="10026729"; 营业部代码 |
| page_size | int | N  | page_size=50; 每页显示的数据条数 |
...
stock_lhb_yyb_detail_em_df = ak.stock_lhb_yyb_detail_em(dept_code="10026729")
...
```
   序号     营业部代码       营业部名称  营业部简称        交易日期    股票代码  股票名称    涨跌幅      买入金额      卖出金额        净额      上榜原因   1日后涨跌幅   2日后涨跌幅  ...
0   1  10026729  中信证券北京总部  中信北京  2024-02-23  301378  通达海   5.52  2498800.0  23187600.0  -20688800  连续三个交易日内     2.31     -3.21  ...
1   2  10026729  中信证券北京总部  中信北京  2024-02-22  301378  通达海   7.82  3156700.0  19876500.0  -16719800  连续三个交易日内     5.52     2.31  ...
[2 rows x 19 columns]
```

## 每日活跃营业部

接口: stock_lhb_hyyyb_date_em

目标地址: https://data.eastmoney.com/stock/hyyyb.html

描述: 获取东方财富网-数据中心-龙虎榜单-每日活跃营业部数据

限量: 单次返回指定日期的所有活跃营业部数据

输入参数

| 名称        | 类型  | 必选 | 描述                                |
|-----------|-----|----|------------------------------------|
| date      | str | Y  | 交易日期; e.g., 20250411             |
| page_size | int | N  | 每页显示的数据条数; e.g., 50; 默认 50 条每页 |
...
| 序号      | float64 | -                      |
| 营业部名称   | object  | -                      |
| 上榜后1日涨幅 | float64 | 注意单位: %               |
| 上榜后5日涨幅 | float64 | 注意单位: %               |
| 上榜后10日涨幅| float64 | 注意单位: %               |
| 买入金额    | float64 | 注意单位: 元                |
| 卖出金额    | float64 | 注意单位: 元                |
| 净额      | float64 | 注意单位: 元                |
| 买入股票数   | float64 | -                      |
...
stock_lhb_hyyyb_date_em_df = ak.stock_lhb_hyyyb_date_em(date="20250411", page_size=50)
...
```
      序号                                               营业部名称  上榜后1日涨幅  上榜后5日涨幅  上榜后10日涨幅      买入金额       卖出金额         净额  买入股票数  卖出股票数
0    1.0               西藏东方财富证券股份有限公司拉萨团结路第二证券营业部     0.92     2.15      3.45   54829523   23548215   31281308     2.0     1.0
1    2.0                     华鑫证券有限责任公司宁波沧海路证券营业部     1.23     2.89      4.12   48521632   19854632   28667000     3.0     2.0
...  ...                                               ...      ...      ...       ...        ...        ...        ...     ...     ...
```
