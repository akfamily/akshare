...
可修改范围经过严格限定, 这是本脚本的核心设计:
    - .py 只处理 docstring 与 # 注释, 其余字符串常量一律跳过。原因是中文列名
      ...
      解析上游文本的字面量（.split("："）) 里的标点属于数据或对外契约, 改动会
      直接破坏接口与用户代码。
...
# 历史发布记录默认不动: 条目是不可变的历史事实, 追溯改写会让跨版本 diff 与
# git blame 产生大量噪声。需要一并统一时传 --include-changelog。
CHANGELOG_PATHS = {"docs/changelog.md"}
...
def editable_spans(source: str) -> List[Tuple[int, int, bool]]:
    ...
    :param source: 源码文本
    :return: 按起点升序排列的 (start, end, 是否字符串常量) 列表
    """
...
def iter_targets(paths: List[str], include_changelog: bool) -> List[pathlib.Path]:
    """
    展开待处理的文件列表。

    :param paths: 命令行给出的路径, 为空则扫描 akshare 与 docs
    :param include_changelog: 是否包含历史发布记录
    :return: 文件路径列表
    """
    ...
    for item in candidates:
        ...
        try:
            ...
            rel = item.as_posix()
        if not include_changelog and rel in CHANGELOG_PATHS:
            continue
        result.append(item)
    return result
...
def main() -> int:
    ...
    parser.add_argument(
        "--include-changelog", action="store_true", help="一并处理历史发布记录"
    )
    ...
    for path in iter_targets(args.paths, args.include_changelog):
        try:
