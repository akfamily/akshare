...
def stock_zh_ah_daily(
    symbol: str = "02318", start_year: str = "2000", end_year: str = "2019"
) -> pd.DataFrame:
    """
    腾讯财经-港股-AH-股票历史行情
    ...
    for year in tqdm(range(int(start_year), int(end_year))):
        ...
        res = requests.get(
            hk_stock_url, params=hk_stock_payload_copy, headers=hk_stock_headers
        )
        data_json = demjson.decode(
            res.text[res.text.find("{") : res.text.rfind("}") + 1]
        )
...
if __name__ == "__main__":
    ...
    for item in big_dict.keys():
        stock_zh_ah_daily_df = stock_zh_ah_daily(
            symbol=item, start_year="2000", end_year="2019"
        )
