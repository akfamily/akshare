...
def stock_hold_change_cninfo(symbol: str = "全部") -> pd.DataFrame:
    ...
    https://webapi.cninfo.com.cn/#/thematicStatistics
    :param symbol: choice of {"深市主板", "沪市", "创业板", "科创板", "北交所", "全部"}; 从 2010 开始
    :type symbol: str
    :return: 实际控制人持股变动
    :rtype: pandas.DataFrame
    """
    ...
    #https://webapi.cninfo.com.cn/api/sysapi/p_sysapi1029?market=012002
    ...
    temp_df["变动日期"] = pd.to_datetime(temp_df["变动日期"], errors="coerce").dt.date
    temp_df["公告日期"] = pd.to_datetime(temp_df["公告日期"], errors="coerce").dt.date
    temp_df["总股本(万股)"] = pd.to_numeric(temp_df["总股本(万股)"], errors="coerce")
    temp_df["已流通股份(万股)"] = pd.to_numeric(temp_df["已流通股份(万股)"], errors="coerce")
    temp_df["已流通比例(%)"] = pd.to_numeric(temp_df["已流通比例(%)"], errors="coerce")
    temp_df["流通受限股份(万股)"] = pd.to_numeric(temp_df["流通受限股份(万股)"], errors="coerce")
    return temp_df


if __name__ == "__main__":    
    stock_hold_change_cninfo_df = stock_hold_change_cninfo(
        symbol="全部"
    )
