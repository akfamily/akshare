...
for file_name in dict_list_name:
    ...
    file_df = pd.read_csv(
        file_name, encoding="gb2312", parse_dates=["日期"], index_col=["日期"]
    )
...
len(fit_list)
sh_df = pd.read_csv(
    fit_list[4], encoding="gb2312", parse_dates=["日期"], index_col=["日期"]
)
...
for file_name in fit_list:
    ...
    # plt.show()
    df_use = pd.DataFrame(
        round(df_use["百分比变化"].str.replace("%", "").astype(float) / 100, 6)
    )
