...
for item_1 in web_site.iloc[:, 0]:
    ...
    if title == "科威特股市指数":
        continue
    useful_web = (
        soup.find_all(attrs={"id": "cr1"})[0]
        .find_all(attrs={"class": "bold left noWrap elp plusIconTd"})[0]
        .select("a")[0]["href"]
    )
    useful_title = (
        soup.find_all(attrs={"id": "cr1"})[0]
    ...
    end_date = "2019/10/17"
    para_header = soup.find_all(attrs={"class": "float_lang_base_1 inlineblock"})[
        0
    ].get_text()
    headers = {
    ...
    vest_time = [
        item.get_text()
        for item in soup.find_all("td", attrs={"class": "first left bold noWrap"})
    ]
    ...
    df_data.to_csv(
        "C:\\Users\\king\\Desktop\\obor_rank\\data\\"
        + useful_title
        + "--"
        + title
        + ".csv",
