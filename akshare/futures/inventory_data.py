...
code_temp_list = [
    item["value"] for item in soup.find_all("select")[0].find_all("option")
]
name_temp_list = [
    item.get_text() for item in soup.find_all("select")[0].find_all("option")
]
code_exchange_name_dict = dict(zip(code_temp_list, name_temp_list))
exchange_value = soup.find_all("select")[0].find_all(attrs={"selected": "selected"})[0][
    "value"
]
symbol_list = soup.find_all("select")[1].get_text().split("\n")[1:-1]
symbol_value_list = [
    item["value"] for item in soup.find_all("select")[1].find_all("option")
]
...
for i in code_temp_list:
    ...
    while j < 5:
        try:
            ...
            print(exchange)
            exchange_value = soup.find_all("select")[0].find_all(
