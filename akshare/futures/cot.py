...
def get_rank_sum(date=None, vars_list=cons.contract_symbols):
    ...
    if len(big_dict.items()) > 0:
        add_vars = [
            i
            for i in cons.market_exchange_symbols["shfe"]
...
def get_czce_rank_table(date="20081015", vars_list=cons.contract_symbols):
    ...
    if date <= datetime.date(2010, 8, 25):
        ...
        for i in range(len(symbols)):
            ...
            table_cut.loc[
                "合计", ["vol_party_name", "long_party_name", "short_party_name"]
            ] = None
    ...
    with zipfile.ZipFile(BytesIO(r.content), "r") as z:
        for i in z.namelist():
            ...
            try:
                ...
                data = data.iloc[
                    start_list[0] :, data.columns[data.iloc[start_list[0], :].notnull()]
                ]
...
if __name__ == "__main__":
    # 郑州商品交易所
    get_czce_rank_table_first_df = get_czce_rank_table(
        date="20081015", vars_list=["SR"]
    )
